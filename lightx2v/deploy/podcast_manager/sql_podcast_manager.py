...
class SQLPodcastManager:
        ...
        try:
            if user_id:
                row = await conn.fetchrow(
                    f"SELECT * FROM {self.table_podcasts} WHERE session_id = $1 AND user_id = $2 AND tag != 'delete'",
                    session_id, user_id
                )
            else:
                row = await conn.fetchrow(
                    f"SELECT * FROM {self.table_podcasts} WHERE session_id = $1 AND tag != 'delete'",
                    session_id
                )
    ...
    async def delete_podcast(self, session_id: str, user_id: str) -> bool:
        ...
        try:
            await conn.execute(
                f"UPDATE {self.table_podcasts} SET tag = 'delete', updated_at = $1 WHERE session_id = $2 AND user_id = $3",
