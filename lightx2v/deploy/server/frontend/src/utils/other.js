...
export const locale = i18n.global.locale
        ...
        const useTemplate = async (item) => {
            ...
            try {
                ...
                if (item.inputs && item.inputs.input_image) {
                    const imageUrl = getTemplateFileUrl(item.inputs.input_image,'images')
                ...
                if (item.inputs && item.inputs.input_audio) {
                    const audioUrl = getTemplateFileUrl(item.inputs.input_audio,'audios')
        ...
        const openTaskDetailModal = (task) => {
            ...
            viewTaskDetail(task);
            // 更新URL路由
            router.push({ name: 'TaskDetail', params: { taskId: task.task_id } });
        };
